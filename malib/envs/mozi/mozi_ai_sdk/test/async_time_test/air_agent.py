...
class CAgent():
    ...
    def test(self,scenario):
        for side_k , side_v in scenario.get_sides().items():
                    ...
                    if self.mission_num == 0:
                        ...
                        for k,v in strike.items():
                            ...
                            for contact_k, contact_v in side_v.contacts.items():
                                i+=1
                                if i== 4:
                                    v.strike_addTarget('["'+contact_k+'"]')
                                    v.strike_removeTarget('["'+contact_k+'"]')
                            v.strike_get_targets()
                            v.strike_setPreplan(True)
                            v.strike_setStrikeMax(2)
                            v.strike_setStrikeFlightSize(2)
                            v.strike_setStrikeMinAircraftReq(2)
                            v.strike_setStrikeRadarUsage(2)
                            v.strike_setStrikeFuleAmmo(2)
                    # 条令
                    if self.doctrine_num == 0:
                        #设置电磁管控是否与上级一致
                        doctrine.doctrine_set_emcon_inherit(False)
                        #设置对空武器使用规则
                        doctrine.set_doctrine_air(air_v.m_Side,1)
                        # 设置海武器使用规则
                        doctrine.set_doctrine_surface(air_v.m_Side,1)
                        # 设置对潜武器使用规则
                        doctrine.set_doctrine_subsurface(air_v.m_Side,1)
                        # 设置对陆武器使用规则
                        doctrine.set_doctrine_land(air_v.m_Side,1)
                        doctrine.set_in_line_with_superiors('no')
                    # 动作
                    #设置一个航路点
            ...
            '''
            if sair_v.num ==1:
                situation = sair_v.scenario.situation
                print(situation)
                sair_v.num = 2
            '''
            scenario.mozi_server.run_simulate()
            # sair_v.mozi_server.run_simulate()
            # sair_v.mozi_server.run_simulate()