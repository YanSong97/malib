...
from malib.envs.mozi.mozi_ai_sdk.test.dppo.envs.common.utils import *
from malib.envs.mozi.mozi_ai_sdk.test.dppo.envs.spaces.mask_discrete import MaskDiscrete
...
class Task(object):
    def __init__(self, env, scenario, sideName):
        ...
        self.time_zone_combine = list(itertools.product([x for x in range(self.times)], [y for y in range(len(self.defend_zones))]))

        self._actions = list(chain.from_iterable([self._Action('donothing', self._ActionDoNothing, self._DoNothingIsValid),
                                                  ...
                                                  self._Action('attack', self._AttackAntiSurfaceShipMissionAction, self._AttackMissionIsValid)]))
    ...
    def _Action(self, type, function, is_valid):
        if type == 'donothing':
            ...
            for times in range(self.times):
                ...
                # key = random.choice(list(self.asup.keys()))
                # missionUnit = {key: self.asup[key]}
                func_list.append(Function(type=type, function=function(missionName, times, self.offend_zone), is_valid=is_valid()))
            return func_list
        elif type == 'attack':
            func_list = []
            for times in range(5, self.times * 2):
                ...
                func_list.append(Function(type=type, function=function(missionName, times, self.target), is_valid=is_valid()))
            return func_list
        else:
    ...
    def _SetTaskParam(self, mission, kwargs):
        ...
        if 'oneThirdRule' in kwargs_keys:
            mission.set_one_third_rule(self.sideName, kwargs['missionName'], kwargs['oneThirdRule'])
        # 是否对巡逻区外的探测目标进行分析
        if 'chechOpa' in kwargs_keys:
            mission.set_opa_check(self.sideName, kwargs['missionName'], kwargs['chechOpa'])
        # 是否对武器射程内探测目标进行分析
        if 'checkWwr' in kwargs_keys:
            mission.set_wwr_check(self.sideName, kwargs['missionName'], kwargs['checkWwr'])
        # 设置任务的开始和结束时间
    ...
    def _DefensiveAirMissionAction(self, missionName, times, zone):
        def act(missionUnit):
            ...
            mission_time = datetime.datetime.strptime(scen_time, '%m/%d/%Y %H:%M:%S') + datetime.timedelta(minutes=self.delta*times)
            side.add_mission_patrol(missionName, 0, zone)  # 空战巡逻
            ...
            taskParam = {'missionName': missionName, 'missionType': '空战巡逻', 'flightSize': 1, 'checkFlightSize': True,
                         'oneThirdRule': True, 'chechOpa': True, 'checkWwr': True, 'startTime': '%s'%str(mission_time),
            ...
            doctrineParam = {'emc_radar': 'Passive', 'evadeAuto': 'true', 'ignorePlottedCourse': 'yes', 'targetsEngaging': 'true',
                      'ignoreEmcon': 'false', 'weaponControlAir': '0', 'weaponControlSurface': '0', 'fuelStateForAircraft': '0',
                      'fuelStateForAirGroup': '3', 'weaponStateForAircraft': '2001', 'weaponStateForAirGroup': '3'}
    ...
    def _AttackAntiSurfaceShipMissionAction(self, missionName, times, target):
        def act(missionUnit):
            ...
            mission_time = datetime.datetime.strptime(scen_time, '%m/%d/%Y %H:%M:%S') + datetime.timedelta(
                minutes=self.delta * times)
            side.add_mission_strike(missionName, 2)
            ...
            taskParam = {'missionName': missionName, 'missionType': '对海打击', 'flightSize': 1, 'checkFlightSize': True,
                         ...
                         'endTime': '08/09/2020 12:00:00', 'isActive': 'true', 'missionUnit': missionUnit, 'targets': _target}
    ...
    def _CreateOrUpdateOffensivePatrolZone(self):
        ...
        Offensive_rp = {v.strName: {'latitude': v.dLatitude, 'longitude': v.dLongitude} for k, v in side.referencepnts.items()
        ...
        if len(Offensive_rp) == 0:
            side.add_reference_point(self.sideName, zone[0], hostileContactBoundingBox[0]['latitude'], hostileContactBoundingBox[0]['longitude'])
            side.add_reference_point(self.sideName, zone[1], hostileContactBoundingBox[1]['latitude'], hostileContactBoundingBox[1]['longitude'])
            side.add_reference_point(self.sideName, zone[2], hostileContactBoundingBox[2]['latitude'], hostileContactBoundingBox[2]['longitude'])
            side.add_reference_point(self.sideName, zone[3], hostileContactBoundingBox[3]['latitude'], hostileContactBoundingBox[3]['longitude'])
        else:
            set_str_1 = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
                self.sideName, zone[0], hostileContactBoundingBox[0]['latitude'], hostileContactBoundingBox[0]['longitude'])
            self.scenario.mozi_server.send_and_recv(set_str_1)
            set_str_2 = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
                self.sideName, zone[1], hostileContactBoundingBox[1]['latitude'], hostileContactBoundingBox[1]['longitude'])
            self.scenario.mozi_server.send_and_recv(set_str_2)
            set_str_3 = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
                self.sideName, zone[2], hostileContactBoundingBox[2]['latitude'], hostileContactBoundingBox[2]['longitude'])
            self.scenario.mozi_server.send_and_recv(set_str_3)
            set_str_4 = "ScenEdit_SetReferencePoint({{side='{}',name='{}', lat={}, lon={}}})".format(
                self.sideName, zone[3], hostileContactBoundingBox[3]['latitude'], hostileContactBoundingBox[3]['longitude'])
    ...
    def _CreateOrUpdateDenfensivePatrolZone(self):
        ...
        if len(rps) != 5:
            # 巡逻任务1
            side.add_reference_point(self.sideName, 'rp2', rp12mid['latitude'], rp12mid['longitude'])
            side.add_reference_point(self.sideName, 'rp3', rp13mid['latitude'], rp13mid['longitude'])
            side.add_reference_point(self.sideName, 'rp4', rp14mid['latitude'], rp14mid['longitude'])
            side.add_reference_point(self.sideName, 'rp5', rp23mid['latitude'], rp23mid['longitude'])
            side.add_reference_point(self.sideName, 'rp6', rp34mid['latitude'], rp34mid['longitude'])
        elif len(rps) == 5:
            # 巡逻任务1
