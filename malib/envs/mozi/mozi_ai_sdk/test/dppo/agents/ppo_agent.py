...
from malib.envs.mozi.mozi_ai_sdk.test.dppo.agents.utils_tf import explained_variance
from malib.envs.mozi.mozi_ai_sdk.test.dppo.utils.utils import tprint

from malib.envs.mozi.mozi_ai_sdk.test.dppo.envs.env import Environment
from malib.envs.mozi.mozi_ai_sdk.test.dppo.envs import etc
from malib.envs.mozi.mozi_ai_sdk.test.dppo.envs.observations import Features
from malib.envs.mozi.mozi_ai_sdk.test.dppo.envs.tasks import Task

def create_env(side_name):
    env = Environment(etc.SERVER_IP, etc.SERVER_PORT, etc.PLATFORM, etc.SCENARIO_NAME, etc.SIMULATE_COMPRESSION,
                      ...
                      etc.SYNCHRONOUS)
    env.start()
    scenario = env.reset(side_name)
    env = Task(env, scenario, side_name)
    env = Features(env, scenario, side_name)
    return env
...
class Model(object):
  def __init__(self, *, policy, ob_space, ac_space, nbatch_act, nbatch_train,
    ...
    # TODO
    self.writer = tf.summary.FileWriter("./tfevents_ppo/", graph=tf.get_default_graph())


class PPOActor(object):

  def __init__(self, env, side_name, policy, unroll_length, gamma, lam, queue_size=1,
    ...
    self._model = Model(policy=policy,
                        ...
                        vf_coef=0.5,
                        max_grad_norm=0.5)
    # self._obs = env.reset()     # TODO
    self._obs, _, _, _ = env.step(0)
  ...
  def _nstep_rollout(self, num):
    ...
    for _ in range(self._unroll_length):
      ...
      # TODO 改成墨子环境observation_wrappers、action_wrappers、reward_wrappers
      self._obs, reward, self._done, info = self._env.step(action[0])
      self._cum_reward += reward
      if self._done:
          # TODO 改成墨子环境observation_wrappers、action_wrappers、reward_wrappers
        if num % 1 == 0:
            self._env = create_env(self._side_name)
...
class PPOLearner(object):
  ...
  def run(self):
    ...
    #   time.sleep(1)
    while len(self._episode_infos) < 75:
      time.sleep(1)
    ...
    while True:
      ...
      loss_tuple = self._model.train(lr_now, clip_range_now, obs, returns, dones, actions, values, neglogpacs, states)
