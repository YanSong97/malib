...
from malib.envs.mozi.mozi_ai_sdk.test.dppo.agents.utils_tf import CategoricalPd
from malib.envs.mozi.mozi_ai_sdk.test.dppo.agents.utils_tf import fc, lstm, batch_to_seq, seq_to_batch
...
class LstmPolicy(object):
  ...
  def __init__(self, sess, scope_name, ob_space, ac_space, nbatch,
    ...
    def value(ob, state, done):
      if isinstance(ac_space, MaskDiscrete):
        return sess.run(vf, {X:ob[0], MASK:ob[-1], STATE:state, DONE:mask})
      else:
        return sess.run(vf, {X:ob, STATE:state, DONE:mask})
