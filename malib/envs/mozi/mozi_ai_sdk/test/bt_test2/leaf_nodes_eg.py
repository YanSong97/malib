...
def update_patrol_mission(scenario):
    ...
    for patrol_mission in patrol_missions:
        if patrol_mission.strName == 'xl1':
            ...
            patrol_mission.set_flight_size('红方', patrol_mission.strName, 1)
            # 如果任务没有设置航线，就添加航线
...
def create_patrol_mission(scenario):
    ...
    for point in point_list:
        ...
        if patrol_name in patrol_mission_name:
            ...
            patrolmssn.set_one_third_rule('红方', patrol_name, 'true')
        i += 1
    return False
...
def create_patrol_zone(scenario):
    ...
    for k, v in ships.items():
        ...
        if '驱逐舰' in v.strName:
            ...
            # xl1
            rp1 = side.add_reference_point('红方', 'rp1', lat + 1.2, lon + 1)
            rp2 = side.add_reference_point('红方', 'rp2', lat + 1, lon + 1.5)
            rp3 = side.add_reference_point('红方', 'rp3', lat + 0.6, lon + 1)
            rp4 = side.add_reference_point('红方', 'rp4', lat + 1, lon + 0.5)
            point_list.append([rp1, rp2, rp3, rp4])
            # xl2
            rp5 = side.add_reference_point('红方', 'rp5', lat - 0.7, lon + 1)
            rp6 = side.add_reference_point('红方', 'rp6', lat - 1, lon + 1.5)
            rp7 = side.add_reference_point('红方', 'rp7', lat - 1.2, lon + 1)
            rp8 = side.add_reference_point('红方', 'rp8', lat - 1, lon + 0.6)
            point_list.append([rp5, rp6, rp7, rp8])
            # xl3
            rp13 = side.add_reference_point('红方', 'rp13', lat + 0.25, lon - 1)
            rp14 = side.add_reference_point('红方', 'rp14', lat + 0.25, lon - 0.3)
            rp15 = side.add_reference_point('红方', 'rp15', lat - 0.25, lon - 0.3)
            rp16 = side.add_reference_point('红方', 'rp16', lat - 0.25, lon - 1)
    ...
    for contact in contacts.values():
        if '驱逐舰' in contact.strName:
            ...
            rp9 = side.add_reference_point('红方', 'rp9', lat1 + 0.7, lon1 - 0.5)
            rp10 = side.add_reference_point('红方', 'rp10', lat1 + 0.7, lon1 + 0.5)
            rp11 = side.add_reference_point('红方', 'rp11', lat1 - 0.7, lon1 + 0.5)
            rp12 = side.add_reference_point('红方', 'rp12', lat1 - 0.7, lon1 - 0.5)
