...
class BT:
    ...
    def sequence(self, scenario):
        for k, v in self.children.items():
            if self.wrap(v.run(scenario)) == self.results['fail']:
                return self.results['fail']
        return self.results['success']

    def select(self, scenario):
        for k, v in self.children.items():
            if self.wrap(v.run(scenario)) == self.results['success']:
                return self.results['success']
        return self.results['fail']
    ...
    def repeat_until_fail(self, scenario):
        while self.wrap(self.children[1].run(scenario)) != self.results['fail']: 1
        return self.results['success']
