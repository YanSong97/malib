...
from malib.envs.mozi.mozi_ai_sdk.env import base_env
import numpy as np
from malib.envs.mozi.mozi_utils import pylog
from malib.envs.mozi.mozi_utils.geo import get_point_with_point_bearing_distance
from malib.envs.mozi.mozi_utils.geo import get_degree
from malib.envs.mozi.mozi_utils.geo import get_two_point_distance
...
class Env(base_env.BaseEnvironment):
    """
    环境类
    """

    def __init__(self, IP, AIPort, scenario_name, simulate_compression, duration_interval,server_plat):
        super(Env, self).__init__(IP, AIPort, server_plat,scenario_name, simulate_compression, duration_interval)
    ...
    def _execute_action(self, action_value):
        ...
        for guid in airs:
            ...
            if self._check_is_find_target():
                ...
                aircraft.auto_attack_target(contact_target_guid)
            else:
                if distance < etc.target_radius:
                    if self._check_is_drop_sonobuoy(action_value):
                        pylog.info("drop_sonobuoy")
                        aircraft.drop_sonobuoy(self.red_side_name, "shallow", "active")

                lon, lat = self._deal_point_data(waypoint)
                aircraft.set_waypoint(lon, lat)
                #print("setwaypoint %s %s" % (lon, lat))
