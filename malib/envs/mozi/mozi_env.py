...
from malib.envs.mozi.remote_handle_docker import _start_mozi_docker, release_docker
...
class _MoZiEnv(ParallelEnv):
    ...
    def __init__(self, **kwargs):
        ...
        while not is_port_available(etc.SERVER_IP, int(etc.SERVER_PORT)):
            port = np.random.randint(50000, 60000)
            etc.SERVER_PORT = str(port)
        print("SERVER_PORT: ", etc.SERVER_PORT)

        client = docker.from_env()
        self.docker_ip_port = _start_mozi_docker(etc.SERVER_IP, client, etc.SERVER_PORT)
