...
def restart_docker(docker_ip_port, password=None, path=None):
    docker_ip = docker_ip_port.split(":")[0]
    if docker_ip == '127.0.0.1' or docker_ip == 'localhost' or docker_ip == get_local_ip() or docker_ip == get_public_ip():
...
def delete_docker(sever_docker_dict, password=None, path=None):
    '''
...
if __name__ == '__main__':
    ...
    if args.mode == 'create':
        ...
        if args.remote_docker_port:
            sever_docker_info = generate_docker(sever_docker_dict, password=args.password, path=args.path, remote_docker_port=args.remote_docker_port)
        else:
        ...
        delete_docker(sever_docker_dict, password=args.password, path=args.path)
    elif args.mode == 'restart':
        restart_docker(args.sever_docker_ip_port, password=args.password, path=args.path)
