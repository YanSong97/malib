"""
Single instance run MOZI with marl
"""
...
parser.add_argument(
    "--config", type=str, help="YAML configuration path.",
    default="examples/configs/mpe/ddpg_simple_spread.yaml",
    required=False
)
parser.add_argument(
...
if __name__ == "__main__":
    ...
    for epoch in range(1000):
        ...
        with Log.stat_feedback(
            log=True, logger=_logger, worker_idx="Rollout", global_step=epoch
        ...
        for iid, interface in learners.items():
            with Log.stat_feedback(
                log=True, logger=_logger, worker_idx=iid, global_step=global_step
